...
def pid(pid_segment):
    ...
    for pid_3_repetition in pid_segment(1)(3):
        pid_fields += tab + 'PID-03[{:0>2}]:{}'.format(str(index),str(pid_3_repetition)) + cr_lf
        # pid components
        ...
        for pid_components in pid_3_repetition:
            if pid_components(1) != '':
                pid_fields += tab + tab + 'PID-03[{:0>2}].{}:{}'.format(str(index),str(index_component),str(pid_components)) + cr_lf
            index_component += 1
        index_component = 1
        index +=1
    index = 1
...
def obr(obr_segments, orc_12):
    ...
    for obr_field in obr_segments:
        ...
        if str(obr_field(16)) == str(orc_12):
            obr_fields += 'OBR[{:0>2}]-16 = to ORC-12'.format(str(obr_field(1)), str(orc_12)) + cr_lf
        else:
            obr_fields += 'OBR[{:0>2}]-16 not = to ORC-12'.format(str(obr_field(1)), str(orc_12)) + cr_lf

        obr_fields += 'OBR[{:0>2}]-17:{}'.format(str(obr_field(1)), str(obr_field(17))) + cr_lf
        obr_fields += 'OBR[{:0>2}]-22:{}'.format(str(obr_field(1)), str(obr_field(22))) + cr_lf
        obr_fields += 'OBR[{:0>2}]-25:{}'.format(str(obr_field(1)), str(obr_field(25))) + cr_lf
        # print(len(obr_field))
        if len(obr_field) > 27:
            if obr_field(26)(1) != '':
                obr_fields += 'OBR 26 (Susceptibilities)' + cr_lf
                obr_fields += tab + 'OBR[{:0>2}]-26:{}'.format(str(obr_field(1)), str(obr_field(26))) + cr_lf
                obr_fields += tab + tab + 'OBR[{:0>2}]-26.1:{}'.format(str(obr_field(1)), str(obr_field(26)(1)(1))) + cr_lf
                obr_fields += tab + tab + 'OBR[{:0>2}]-26.2:{}'.format(str(obr_field(1)), str(obr_field(26)(1)(2))) + cr_lf
                obr_fields += tab + tab + 'OBR[{:0>2}]-26.3:{}'.format(str(obr_field(1)), str(obr_field(26)(1)(3))) + cr_lf
            else:
...
def spm(spm_segments):
    ...
    for spm_field in spm_segments:
        spm_fields += 'SPM[{:0>2}]'.format(str(index)) + cr_lf
        spm_fields += 'SPM[{:0>2}]-2.2:{}'.format(str(spm_field(1)), str(spm_field(2)(1)(2))) + cr_lf
        spm_fields += tab + 'SPM[{:0>2}]-2.2.1:{}'.format(str(spm_field(1)), str(spm_field(2)(1)(2)(1))) + cr_lf
        spm_fields += 'SPM[{:0>2}]-4:{}'.format(str(spm_field(1)), str(spm_field(4))) + cr_lf
        spm_fields += 'SPM[{:0>2}]-8:{}'.format(str(spm_field(1)), str(spm_field(8))) + cr_lf
        spm_fields += '' + cr_lf
        index +=1
...
def susceptibility(segments):
    ...
    # susceptibility_fields += tab + 'OBR[{:0>2}]-26:{}'.format(str(obr_field(1)), str(obr_field(26))) + cr_lf
    # susceptibility_fields += tab + tab + 'OBR[{:0>2}]-26.1:{}'.format(str(obr_field(1)), str(obr_field(26)(1)(1))) + cr_lf
    # susceptibility_fields += tab + tab + 'OBR[{:0>2}]-26.2:{}'.format(str(obr_field(1)), str(obr_field(26)(1)(2))) + cr_lf
    # susceptibility_fields += tab + tab + 'OBR[{:0>2}]-26.3:{}'.format(str(obr_field(1)), str(obr_field(26)(1)(3))) + cr_lf
...
def generate_elr_report(parsed_hl7):
    ...
    # Print OBR Segment
    report_content += obr(parsed_hl7.segments('OBR'), parsed_hl7.segments('ORC')(1)(12))
    # Print OBX Segment
